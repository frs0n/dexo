...
final class DiscourseAPI {
    ...
    /// Tracks which proxy port was used to build the current session.
    /// If the port changes (proxy started/stopped), we rebuild the session.
    private var sessionProxyPort: Int32 = -1
    private var _session: Session?

    private var session: Session {
        let currentPort = ProxyManager.shared.port
        if _session == nil || sessionProxyPort != currentPort {
            _session = DiscourseAPI.makeSession(interceptor: interceptor)
            sessionProxyPort = currentPort
        }
        return _session!
    }
    ...
    private static func makeSession(interceptor: DiscourseAuthInterceptor) -> Session {
        ...
        if let proxyDict = ProxyManager.shared.proxyConfiguration {
            config.connectionProxyDictionary = proxyDict
        }
        // Disable automatic cookie handling — web auth cookies are managed by WebCookieStore
        // and injected manually in the interceptor. Mixing them causes CSRF token mismatch.
        config.httpCookieAcceptPolicy = .never
        config.httpShouldSetCookies = false
        let delegate = ProxyAwareSessionDelegate()
        return Session(configuration: config, delegate: delegate, interceptor: interceptor)
    }
